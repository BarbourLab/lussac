...
def automerge_sortings(data: PhyData, unit_ids: dict, params: dict, plot_folder: str):
	...
	if not kwargs['only_agreed_units']:
		add_leftover_units(data, unit_ids, kwargs, plot_folder) # A unit that is not detected in other analyses is usually not a neuron ; see Buccino & Hurwitz et Al. 2020
...
def check_conflicts(data: PhyData, starting_ID: int, params: dict, plot_folder: str):
	...
	for unit_id in data.merged_sorting.get_unit_ids():
		...
		wvfs = data.get_unit_waveforms(unit_id, **wvf_extraction_kwargs)
